...
from model.network import define_A, define_D, define_G

from utils.utils_losses import L_clip_from_feature, MultiScaleSemanticLoss
...
class ModelPlain(ModelBase):
    ...
    def load(self, init_paths):
        ...
        load_path_D = init_paths[1]
        load_path_G = init_paths[2]
    ...
    def calculate_vectors(self):
        embs_neg1 = self.extract_embs(self.config.lowlight_images_path, model) # [n, 512]
    ...
    def define_optimizer(self):
        ...
        self.train_optimizer = torch.optim.Adam(itertools.chain(A_optim_params, D_optim_params, G_optim_params), lr=self.opt.train_lr, betas=(0.5, 0.999))


    def define_scheduler(self):
        self.lr_scheduler = lr_scheduler.MultiStepLR(self.train_optimizer, milestones=[self.opt.num_epochs/4, self.opt.num_epochs/2, self.opt.num_epochs/4*3], gamma=0.4)
    ...
    def netA_forward(self):
        ...
        self.identity_o, self.correct_o = self.netA(self.over)
    ...
    def netD_forward(self):
        self.align_u, self.align_o = self.netD(self.identity_u, self.identity_o)
        if self.under_g is not None:
            self.align_u_g, self.align_o_g = self.netD(self.identity_u_g, self.identity_o_g)
    ...
    def optimize_parameters(self):
        ...
        loss_fusion = loss_content + 1e-2 * loss_sam + 1e-3 * loss_clip_local + 1e-3 * loss_clip_global
        # loss_fusion = loss_content + 1e-2 * loss_sam
