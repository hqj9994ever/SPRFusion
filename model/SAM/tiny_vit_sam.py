    ...
    def forward(self, x):  # x (B,N,C)
        ...
        attn = (
            ...
            (self.attention_biases[:, self.attention_bias_idxs]
             if self.training else self.ab.to(x.device))
        )
...
class TinyViT(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000,
        ...
        # 这里的 resolution 只作为初始化占位，不再视为真实固定输入尺寸
    ...
    def _pad_to_multiple(self, x, multiple=16):
        """
        将输入 pad 到 multiple 的倍数，避免 H//16, W//16 与 token 数不匹配
        """
        B, C, H, W = x.shape
    ...
    def _set_runtime_resolution(self, layer, h, w):
        """
        动态更新 layer / block / downsample 中依赖 input_resolution 的属性
        """
        if hasattr(layer, 'input_resolution'):
            layer.input_resolution = (h, w)
        ...
        if hasattr(layer, 'downsample') and layer.downsample is not None:
            if hasattr(layer.downsample, 'input_resolution'):
                layer.downsample.input_resolution = (h, w)

    def _tokens_to_nchw(self, x, h, w):
        # x: [B, N, C] -> [B, C, H, W]
        B, N, C = x.shape
    ...
    def forward_two_stage(self, x):
        """
        只提取前两层特征，然后直接返回，不再经过后面的 layer2/layer3/neck

        return:
            feat_s4: [B, 64, H/4, W/4]
            feat_s8: [B, 128, H/8, W/8]
        """
        x, _, _, _, _ = self._pad_to_multiple(x, multiple=16)
        _, _, H_pad, W_pad = x.shape

        # patch embed -> /4
        x = self.patch_embed(x)
        h0, w0 = H_pad // 4, W_pad // 4

        # layer0
        self._set_runtime_resolution(self.layers[0], h0, w0)
        feat_s4, x = self.layers[0](x, return_before_downsample=True)
        # feat_s4: BCHW, /4, channel=64
        # x: /8

        # layer1
        h1, w1 = h0 // 2, w0 // 2
        self._set_runtime_resolution(self.layers[1], h1, w1)
        feat_s8, x = self.layers[1](x, return_before_downsample=True)
        # feat_s8: [B, N, C] -> BCHW
        if feat_s8.dim() == 3:
            feat_s8 = self._tokens_to_nchw(feat_s8, h1, w1)

        return feat_s4, feat_s8

    def forward_features(self, x, return_multiscale=False):
        x, _, _, _, _ = self._pad_to_multiple(x, multiple=16)
        _, _, H_pad, W_pad = x.shape
        ...
        # layer0: before downsample = /4, after downsample = /8
        self._set_runtime_resolution(self.layers[0], h0, w0)
        feat_s4, x = self.layers[0](x, return_before_downsample=True)
        ...
        # BasicLayer 的 feat_s8 一般是 [B, N, C]，转成 BCHW
        if feat_s8.dim() == 3:
            feat_s8 = self._tokens_to_nchw(feat_s8, h1, w1)

        # x 现在已经是 /16
        h2, w2 = h1 // 2, w1 // 2
        self._set_runtime_resolution(self.layers[2], h2, w2)
        x = self.layers[2](x)

        h3, w3 = h2, w2
        self._set_runtime_resolution(self.layers[3], h3, w3)
        x = self.layers[3](x)

        # 最终 /16
        if x.dim() == 3:
            feat_s16 = self._tokens_to_nchw(x, h3, w3)
        else:
            feat_s16 = x

        feat_s16 = self.neck(feat_s16)

        if return_multiscale:
            return feat_s4, feat_s8, feat_s16

        return feat_s16

    def forward(self, x, return_two_stage=False):
        if return_two_stage:
