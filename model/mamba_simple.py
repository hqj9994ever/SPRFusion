...
class Mamba(nn.Module):
    def __init__(
        ...
        if bimamba_type == "v1":
        ...
        elif bimamba_type == "v2" or bimamba_type=='v3' or bimamba_type=='m3':
            A_b = repeat(
                torch.arange(1, self.d_state + 1, dtype=torch.float32, device=device),
                "n -> d n",
                d=self.d_inner,
            ...
            self.in_proj_extra1 = nn.Linear(self.d_model, self.d_inner * 2, bias=bias, **factory_kwargs)
            self.in_proj_extra2 = nn.Linear(self.d_model, self.d_inner * 2, bias=bias, **factory_kwargs)
    ...
    def forward(self, hidden_states, inference_params=None,extra_emb1=None,extra_emb2=None):
        ...
        if inference_params is not None:
            ...
            if inference_params.seqlen_offset > 0:
                ...
                return out
        if extra_emb1 is None and extra_emb2 is None:
            ...
            if self.in_proj.bias is not None:
                xz = xz + rearrange(self.in_proj.bias.to(dtype=xz.dtype), "d -> d 1")
            extra_emb1 = rearrange(
                self.in_proj_extra1.weight @ rearrange(extra_emb1, "b l d -> d (b l)"),
                "d (b l) -> b d l",
                l=seqlen,
            )
            if self.in_proj_extra1.bias is not None:
                extra_emb1 = extra_emb1 + rearrange(self.in_proj_extra1.bias.to(dtype=xz.dtype), "d -> d 1")
            ...
            extra_emb2 = rearrange(
                self.in_proj_extra2.weight @ rearrange(extra_emb2, "b l d -> d (b l)"),
                "d (b l) -> b d l",
                l=seqlen,
            )
            if self.in_proj_extra1.bias is not None:
                extra_emb1 = extra_emb1 + rearrange(self.in_proj_extra1.bias.to(dtype=xz.dtype), "d -> d 1")
            if self.in_proj_extra2.bias is not None:
                extra_emb2 = extra_emb2 + rearrange(self.in_proj_extra2.bias.to(dtype=xz.dtype), "d -> d 1")
        ...
        if self.use_fast_path and inference_params is None:  # Doesn't support outputting the states
            if self.bimamba_type == "v1":
                ...
                # F.linear(rearrange(out_z, "b d l -> b l d"), out_proj_weight, out_proj_bias)
                if not self.if_devide_out:
                    out = F.linear(rearrange(out + out_b + out_c, "b d l -> b l d"), self.out_proj.weight, self.out_proj.bias)
                else:
                    out = F.linear(rearrange(out + out_b + out_c, "b d l -> b l d") / 3, self.out_proj.weight, self.out_proj.bias)
            else:
                out = mamba_inner_fn(
                    xz,
                    self.conv1d.weight,
