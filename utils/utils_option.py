...
def parse(opt_path, is_train=True):
    ...
    # default setting for optimizer
    # ----------------------------------------
    if 'R_optimizer_type' not in opt['train']:
        opt['train']['R_optimizer_type'] = "adam"
    if 'R_optimizer_betas' not in opt['train']:
        opt['train']['R_optimizer_betas'] = [0.9,0.999]
    if 'R_scheduler_restart_weights' not in opt['train']:
        opt['train']['R_scheduler_restart_weights'] = 1
    if 'R_optimizer_wd' not in opt['train']:
        opt['train']['R_optimizer_wd'] = 0
    if 'R_optimizer_reuse' not in opt['train']:
        opt['train']['R_optimizer_reuse'] = False
    ...
    if 'R_param_strict' not in opt['train']:
        opt['train']['R_param_strict'] = True
...
def find_last_checkpoint(save_dir, net_type='G', pretrained_path=None):
    ...
    if file_list:
        ...
        for file_ in file_list:
            epoch_current = re.findall(r"epoch_(\d+)_{}.pth".format(net_type), file_)
            epoch_exist.append(int(epoch_current[0]))
        init_epoch = max(epoch_exist)
        init_path = os.path.join(save_dir, 'epoch_{}_{}.pth'.format(init_epoch, net_type))
    else:
        init_epoch = 0
        init_path = pretrained_path
    return init_epoch, init_path
...
def save(opt):
    ...
    with open(dump_path, 'w') as dump_file:
        json.dump(opt, dump_file, indent=2)  # dict -> JSON
